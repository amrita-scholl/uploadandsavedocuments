...
headers = {"Authorization": "Bearer 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"}
...
def save_relationship(doc_id: str, chunk_ids: List[str],chunks:str):
     with neo4j_driver.session() as session:
         print(f"Creating doc_id: {doc_id} ")
         session.run("CREATE (d:Document {doc_id: $doc_id})", doc_id=doc_id)
         for chunk_id in chunk_ids:
             session.run("CREATE (c:Chunk {chunk_id: $chunk_id})", chunk_id=chunk_id)
             session.run("MATCH (d:Document {doc_id: $doc_id}), (c:Chunk {chunk_id: $chunk_id}) "
                         "CREATE (d)-[:CONTAINS]->(c)", doc_id=doc_id, chunk_id=chunk_id)
         for chunk_id,chunk_text in zip(chunk_ids, chunks):
             print(f"Creating ChunkText with id: {chunk_id} and text: {chunk_text}")
             session.run("CREATE (c:ChunkText {chunk_text: $chunk_text})", chunk_text=chunk_text)
             session.run("MATCH (c:Chunk {chunk_id: $chunk_id}), (ct:ChunkText {chunk_text: $chunk_text})"
                         "CREATE (c)-[:HAS_TEXT]->(ct)", chunk_id=chunk_id,chunk_text=chunk_text)
...
async def upload(file: UploadFile = File(...), chunk_size: int = 512):
    try:
        ...
        save_relationship(doc_id, chunk_ids,chunks)
